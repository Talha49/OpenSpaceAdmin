...
export async function POST(req) {
  try {
    ...
    const users = await req.json(); // Parse request body
    ...
    // Filter new users (remove existing ones)
    const newUsers = users
      .filter((user) => !existingEmails.includes(user.email))
      .map((user) => {
        const plainPassword = generateRandomPassword(); // Generate a 6-digit password
        return {
          fullName: user.name,
          email: user.email,
          ...
          password: bcrypt.hashSync(plainPassword, 10), // Hash the password
          plainPassword, // Store plain password temporarily to send in email
        };
      });

    if (newUsers.length === 0) {
      return NextResponse.json(
        { message: "No new users to import.", insertedUsers: [] },
        { status: 200 }
      );
    }
    ...
    for (const user of newUsers) {
      sendEmail(user.email, user.plainPassword, user.name);
    }

    // Return only inserted users (excluding plain passwords)
    const responseUsers = insertedUsers.map(({ _id, name, email }) => ({
      _id,
      name,
      email,
    }));
    ...
    return NextResponse.json(
      { message: "Users imported successfully.", insertedUsers },
      { status: 200 }
    );
  } catch (error) {
