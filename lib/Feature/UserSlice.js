...
export const fetchDeletedUsers = createAsyncThunk('user/fetchDeletedUsers', async () => {
...
export const storeDeletedUser = createAsyncThunk('user/storeDeletedUser', async (user) => {
  const response = await fetch("/api/storeDeletedUser", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export const selectPaginatedDeletedUsers = (state) => {
  const { deletedUsers, deletedUsersCurrentPage, deletedUsersItemsPerPage } = state.user;
...
export const selectDeletedUsersTotalPages = (state) => {
  return Math.ceil(state.user.deletedUsers?.length / state.user.deletedUsersItemsPerPage);
};
export const selectFilteredUsers = (state) => {
  ...
  if (sortOrder !== null) {
    filteredUsers.sort((a, b) => {
      ...
      return sortOrder ? firstLetterA.localeCompare(firstLetterB) : firstLetterB.localeCompare(firstLetterA);
    });
  }
